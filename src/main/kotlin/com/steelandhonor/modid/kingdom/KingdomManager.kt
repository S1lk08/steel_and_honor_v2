...
object KingdomManager {
    ...
    private fun collectCitySnapshots(server: MinecraftServer): List<CityBorderSnapshot> {
        ...
        perOwner.entries.sortedBy { kingdoms[it.key]?.name ?: it.key.toString() }.forEach { (owner, list) ->
            ...
            trimmed.forEach { snapshot ->
                val area = ClaimedArea(owner, snapshot.dimension.value, snapshot.minChunkX, snapshot.maxChunkX, snapshot.minChunkZ, snapshot.maxChunkZ)
        ...
        runCatching {
            Files.newInputStream(path).use { input ->
                ...
                list.forEach { element ->
                    ...
                    val partyId = if (element.containsUuid("PartyId")) element.getUuid("PartyId") else null
                    val claimCount = if (element.contains("ClaimCount", NbtElement.INT_TYPE.toInt())) element.getInt("ClaimCount") else 0
                ...
                nbt.getList("Wars", NbtElement.COMPOUND_TYPE.toInt()).forEach { warTag ->
                    if (warTag is NbtCompound && warTag.containsUuid("Attacker") && warTag.containsUuid("Defender")) {
                        ...
                        val attackerKills = if (warTag.contains("AttackerKills", NbtElement.INT_TYPE.toInt())) warTag.getInt("AttackerKills") else 0
                        val defenderKills = if (warTag.contains("DefenderKills", NbtElement.INT_TYPE.toInt())) warTag.getInt("DefenderKills") else 0
                        val startTick = if (warTag.contains("StartTick", NbtElement.LONG_TYPE.toInt())) warTag.getLong("StartTick") else 0L
                        wars.add(
                            WarState(
                                warTag.getUuid("Attacker"),
                                warTag.getUuid("Defender"),
                                attackerAllies,
                                defenderAllies,
                                attackerKills,
                                defenderKills,
                ...
                nbt.getList("WarRequests", NbtElement.COMPOUND_TYPE.toInt()).forEach { requestTag ->
                    if (requestTag is NbtCompound &&
                        ...
                        val side = runCatching { WarSide.valueOf(requestTag.getString("Side")) }.getOrNull() ?: return@forEach
                        warJoinRequests.add(
                            WarJoinRequest(
                                requestTag.getUuid("Requester"),
                                requestTag.getUuid("Attacker"),
                                requestTag.getUuid("Defender"),
                                side
                            )
                        )
                    }
                ...
                nbt.getList("Invites", NbtElement.COMPOUND_TYPE.toInt()).forEach { inviteTag ->
                    if (inviteTag is NbtCompound && inviteTag.containsUuid("Player") && inviteTag.containsUuid("Kingdom")) {
    ...
    fun assignRole(executor: ServerPlayerEntity, target: ServerPlayerEntity, role: KingdomRole) {
        val owner = memberLookup[executor.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        ...
        val kingdom = kingdoms[owner] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
    ...
    fun invitePlayer(executor: ServerPlayerEntity, target: ServerPlayerEntity) {
        ...
        if (memberLookup[target.uuid] != null) throw KingdomOperationException("command.steel_and_honor.kingdom.invite.already_member")
        ...
        target.sendMessage(Text.translatable("command.steel_and_honor.kingdom.invite.notify", kingdom.name), false)
        dirty = true
    }
    ...
    fun joinKingdom(player: ServerPlayerEntity, kingdomName: String) {
        if (memberLookup[player.uuid] != null) throw KingdomOperationException("command.steel_and_honor.kingdom.join.already_member")
        ...
        val pending = invites[player.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.join.no_invite")
        ...
        dirty = true
        markBordersDirty()
        player.sendMessage(Text.translatable("command.steel_and_honor.kingdom.join.success", entry.value.name), false)
        notifyMembers(player.server, entry.value, Text.translatable("command.steel_and_honor.kingdom.join.announce", player.gameProfile.name, entry.value.name))
        applyToPlayer(player)
    }

    fun leaveKingdom(player: ServerPlayerEntity) {
        val owner = memberLookup[player.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        val kingdom = kingdoms[owner] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
    ...
    fun updateKingdomColor(player: ServerPlayerEntity, color: DyeColor): DyeColor {
        ...
        if (kingdom.color == color) throw KingdomOperationException("command.steel_and_honor.kingdom.color.same")
        kingdom.color = color
        dirty = true
        markBordersDirty()
    ...
    fun declareWar(executor: ServerPlayerEntity, targetName: String) {
        val attackerOwner = memberLookup[executor.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        val attacker = kingdoms[attackerOwner] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        if (!attacker.isKingdom()) throw KingdomOperationException("command.steel_and_honor.kingdom.war.only_kingdom")
        ...
        if (!defenderEntry.value.isKingdom()) throw KingdomOperationException("command.steel_and_honor.kingdom.war.only_kingdom")
        if (defenderEntry.key == attackerOwner) throw KingdomOperationException("command.steel_and_honor.kingdom.war.self")
        if (isInWar(attackerOwner)) throw KingdomOperationException("command.steel_and_honor.kingdom.war.in_progress")
        if (isInWar(defenderEntry.key)) throw KingdomOperationException("command.steel_and_honor.kingdom.war.target_busy")
        if (isAtWar(attackerOwner, defenderEntry.key)) throw KingdomOperationException("command.steel_and_honor.kingdom.war.exists")
        wars.add(WarState(attackerOwner, defenderEntry.key, startTick = executor.server.overworld.time))
        dirty = true
    ...
    fun requestWarAssistance(player: ServerPlayerEntity, allyName: String): String {
        val requesterOwner = memberLookup[player.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        val requesterKingdom = kingdoms[requesterOwner] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        if (!requesterKingdom.isKingdom()) throw KingdomOperationException("command.steel_and_honor.kingdom.war.only_kingdom")
        if (isInWar(requesterOwner)) throw KingdomOperationException("command.steel_and_honor.kingdom.war.request.ineligible")
        val allyEntry = kingdoms.entries.firstOrNull { it.value.name.equals(allyName, true) }
            ?: throw KingdomOperationException("command.steel_and_honor.kingdom.war.unknown")
        if (allyEntry.key == requesterOwner) throw KingdomOperationException("command.steel_and_honor.kingdom.war.request.self")
        ...
        if (war.includes(requesterOwner)) throw KingdomOperationException("command.steel_and_honor.kingdom.war.request.ineligible")
        if (warJoinRequests.any { it.requester == requesterOwner && it.attacker == war.attacker && it.defender == war.defender }) {
        ...
        dirty = true
        notifyMembers(player.server, allyEntry.value, Text.translatable("command.steel_and_honor.kingdom.war.request.notify", requesterKingdom.name))
        notifyMembers(player.server, requesterKingdom, Text.translatable("command.steel_and_honor.kingdom.war.request.sent", allyEntry.value.name))
        return allyEntry.value.name
    }

    fun respondToWarRequest(player: ServerPlayerEntity, requesterName: String, approve: Boolean): Pair<String, String> {
        val owner = memberLookup[player.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        val responderKingdom = kingdoms[owner] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        if (!getRole(player.uuid).isCommandRank) throw KingdomOperationException(GENERIC_PERMISSION_KEY)
        val request = warJoinRequests.firstOrNull {
            it.targetOwner() == owner && kingdoms[it.requester]?.name?.equals(requesterName, true) == true
        ...
        if (!approve) {
            warJoinRequests.remove(request)
            dirty = true
            notifyMembers(player.server, requesterKingdom, Text.translatable("command.steel_and_honor.kingdom.war.request.denied", responderKingdom.name))
        ...
        notifyMembers(player.server, requesterKingdom, Text.translatable("command.steel_and_honor.kingdom.war.request.approved", responderKingdom.name, opponentName))
        notifyMembers(player.server, responderKingdom, Text.translatable("command.steel_and_honor.kingdom.war.request.helper_joined", requesterKingdom.name))
        return requesterKingdom.name to responderKingdom.name
    }
    ...
    fun forceAssignToKingdom(target: ServerPlayerEntity, kingdomName: String): String {
        ...
        target.sendMessage(Text.translatable("command.steel_and_honor.kingdom.join.force", destination.name), false)
        return destination.name
    }

    fun updateDesign(player: ServerPlayerEntity, transform: (KingdomDesign) -> KingdomDesign): Boolean {
        val ownerId = memberLookup[player.uuid] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
        val kingdom = kingdoms[ownerId] ?: throw KingdomOperationException("command.steel_and_honor.kingdom.not_member")
    ...
    fun statusTextFor(playerId: UUID): Text {
        val owner = memberLookup[playerId] ?: return Text.translatable("command.steel_and_honor.kingdom.status.unaligned")
        val data = kingdoms[owner] ?: return Text.translatable("command.steel_and_honor.kingdom.status.unaligned")
...
fun recordKill(killer: ServerPlayerEntity, victim: ServerPlayerEntity) {
    ...
    private fun applyToMembers(server: MinecraftServer, kingdom: KingdomData) {
        kingdom.members.forEach { uuid ->
            server.playerManager.getPlayer(uuid)?.let { KingdomItemApplier.applyToPlayer(it, kingdom.design) }
        }
    }
    ...
    private fun updateWars(server: MinecraftServer) {
        ...
        wars.forEach { war ->
            ...
            if (war.timeRemaining(now) <= 0L) {
                concluded.add(war)
            }
        }
        concluded.forEach { resolveWar(server, it) }
    }

    private fun resolveWar(server: MinecraftServer, war: WarState) {
    ...
    private fun sendClaimLimitWarning(server: MinecraftServer, kingdom: KingdomData, allowed: Int, attempted: Int) {
        val message = Text.translatable("command.steel_and_honor.kingdom.claim.limit", allowed, attempted, kingdom.name)
        notifyMembers(server, kingdom, message)
    }

    private fun sendOverlapWarning(server: MinecraftServer, kingdom: KingdomData, dimension: Identifier) {
        val message = Text.translatable("command.steel_and_honor.kingdom.claim.overlap", kingdom.name, dimension.toString())
        notifyMembers(server, kingdom, message)
    }

    private fun notifyMembers(server: MinecraftServer, kingdom: KingdomData, message: Text) {
...
data class WarState(
    ...
    fun matches(first: UUID, second: UUID): Boolean {
        return (attacker == first && defender == second) || (attacker == second && defender == first)
    }
    ...
    fun timeRemaining(now: Long): Long {
        if (startTick == 0L) return KingdomManager.WAR_DURATION_TICKS
