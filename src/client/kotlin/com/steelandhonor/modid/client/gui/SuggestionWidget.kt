...
class SuggestionWidget(
    ...
    fun render(context: DrawContext, mouseX: Int, mouseY: Int) {
        ...
        for (i in startIndex until endIndex) {
            ...
            if (isSelected) {
                context.fill(x + 1, itemY - 1, x + actualWidth - 1, itemY + itemHeight - 1, selectedColor)
            }
    ...
    fun isMouseOver(mouseX: Double, mouseY: Double): Boolean {
        ...
        val totalHeight = visibleCount * itemHeight + padding * 2
        val maxWidth = suggestions.maxOfOrNull { MinecraftClient.getInstance().textRenderer.getWidth(it) } ?: width
        val actualWidth = max(width, min(maxWidth + padding * 2, 200)).toDouble()
        
        return mouseX >= x && mouseX <= x + actualWidth && mouseY >= y && mouseY <= y + totalHeight
    }
